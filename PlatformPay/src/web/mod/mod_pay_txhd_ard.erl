...
%%    ?ERR("txhd_ard: ~p~n",[QueryStringT]),
    ...
    case Md52 =:= Md53 of
        false ->
            db_func:log_pay_info(2, 0, 0, 0, erlang:localtime(), 0, ?ACCOUNT_TYPE_TXHD_ARD, QueryStringT),
            Reply = "FAILED",
        ...
        true ->
            ...
            Url = "http://"++Server#server.serverIP++":"++HttpPort++"/pay_txhd_ard/",
            %?DEBUG("Url:~w",[Url]),
            ...
            case Response of
                {ok, {_,_,Content}} ->
                    ...
                    if Result == 1 ->
                           ...
                           db_func:log_pay_info(1, RoleID2, AccID, Amount2, erlang:localtime(), 1, ?ACCOUNT_TYPE_TXHD_ARD, QueryStringT),
                           Reply = "SUCCESS",
                           Req:ok({"text/html; charset=utf-8", Reply});
                       true ->
                           db_func:log_pay_info(3, RoleID2, 0, Amount2, erlang:localtime(), 1, ?ACCOUNT_TYPE_TXHD_ARD, QueryStringT),
                           Reply = "FAILED",
                            Req:ok({"text/html; charset=utf-8", Reply}),
                            ?ERR("txhd_ard check_order failed. reason:gameserver return false,order:~w",[QueryStringT])
                    end;
                _ ->
                    db_func:log_pay_info(4, RoleID2, 0, Amount2, erlang:localtime(), 1, ?ACCOUNT_TYPE_TXHD_ARD, QueryStringT),
                    Reply = "FAILED",
                    Req:ok({"text/html; charset=utf-8", Reply}),
