%% @author liuqiang
%% @doc @todo Add description to pay_mod_mz.
        ...
        false ->
            case check_sign(Username, ChangeID, Money, Hash) of
                true ->
                    ...
                    URL = "http://" ++ Server#server.serverIP ++ ":" ++ HttpPort ++ "/paymz?" ++ SendQS,
                    ...
                    case Response of
                        {ok, {_, _, Content}} ->
                            ...
                            if Result =:= 1 ->
                                    ...
                                    db_func:log_pay_info(1, RoleID, AccID, Amount, erlang:localtime(), 1, ?ACCOUNT_TYPE_MZ, QueryString),
                                    Reply = "1",
                                    Req:ok({"text/html; charset=utf-8", Reply});
                                true ->
                                    db_func:log_pay_info(3, RoleID, 0, Amount, erlang:localtime(), 1, ?ACCOUNT_TYPE_MZ, QueryString),
                                    Reply = "0",
                                    Req:ok({"text/html; charset=utf-8", Reply}),
                                    ?ERR("mz check_order failed. reason:game server return false,order:~w",[QueryString])
                            end;
                        _ ->
                            db_func:log_pay_info(4, RoleID, 0, Amount, erlang:localtime(), 1, ?ACCOUNT_TYPE_MZ, QueryString),
                            Reply = "0",
                            Req:ok({"text/html; charset=utf-8", Reply}),
                            ?ERR("mz check_order failed. reason:game Server not response not 0,order:~w",[QueryString])
                    end;
                _ ->
                    db_func:log_pay_info(2, 0, 0, 0, erlang:localtime(), 0, ?ACCOUNT_TYPE_MZ, QueryString),
                    Reply = "0",
                    Req:ok({"text/html; charset=utf-8", Reply}),
                    ?ERR("mz check_order failed. reason:sign wrong,order:~w",[QueryString])
            end;
        _ ->
