...
handle(Req, Method, Path)->
	...
	case verify_sign(QueryString, Method, Path) of
		true ->
			...
			Url = "http://"++Server#server.serverIP++":"++HttpPort++"/payqq?" ++ SendQS,
			...
			case Response of
				{ok, {_,_,Content}} ->
					...
					if Result2 == 1 ->
						   ...
						   db_func:log_pay_info(1, RoleID2, AccID, Amount, erlang:localtime(), 1, ?ACCOUNT_TYPE_QQ, QueryString),
						   Reply = ejson:encode({[{<<"result">>,0}]}),
						   Req:ok({"text/html; charset=utf-8", Reply});
					   true ->
						   db_func:log_pay_info(3, RoleID2, 0, Amount, erlang:localtime(), 1, ?ACCOUNT_TYPE_QQ, QueryString),
						   Reply = ejson:encode({[{<<"result">>,94}]}),
						   Req:ok({"text/html; charset=utf-8", Reply}),
						   ?ERR("qq check_order failed. reason:game server return false,order:~w",[QueryString])
					end;
				_ ->
					db_func:log_pay_info(4, RoleID2, 0, Amount, erlang:localtime(), 1, ?ACCOUNT_TYPE_QQ, QueryString),
					Reply = ejson:encode({[{<<"result">>,2}]}), %% 订单未正确获取,需要重新获取订单
					Req:ok({"text/html; charset=utf-8", Reply}),
					?ERR("qq check_order failed. reason:game server no response,order:~w",[QueryString])
			end;
		_ ->
			db_func:log_pay_info(2, 0, 0, 0, erlang:localtime(), 0, ?ACCOUNT_TYPE_QQ, QueryString),
			Reply = ejson:encode({[{<<"result">>,1}]}),
			Req:ok({"text/html; charset=utf-8", Reply}),
			?ERR("qq check_order failed. reason:sign wrong,order:~w",[QueryString])
	end.
