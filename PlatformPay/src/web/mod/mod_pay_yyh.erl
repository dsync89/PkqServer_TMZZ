		...
		true ->
			...
			case get_value(DataList,<<"result">>) of
				0 ->
					...
					Url = "http://"++Server#server.serverIP++":"++HttpPort++"/payyyh?" ++ SendQS,
					%?ERR("Url:~s",[Url]),
						...
						{ok, {_,_,Content}} ->
							...
							if Result == 1 ->
							   ...
							   true ->
								   ...
								   Reply = "FAILED",
								   Req:ok({"text/html; charset=utf-8", Reply}),
								   ?ERR("yyh check_order failed. reason:game server return false,order:~w",[QueryString])
							end;
						_ ->
							db_func:log_pay_info(4, RoleID2, 0, Amount, erlang:localtime(), 1, ?srcType, QueryString),
							Reply = "FAILED", %% 订单未正确获取,需要重新获取订单
							Req:ok({"text/html; charset=utf-8", Reply}),
							?ERR("yyh check_order failed. reason:game Server not response not 0,order:~w",[QueryString])
					end;
				_ ->
					db_func:log_pay_info(5, 0, 0, 0, erlang:localtime(), 0, ?srcType, QueryString),
					Reply = "FAILED",
					Req:ok({"text/html; charset=utf-8", Reply}),
					?ERR("yyh check_order failed. reason:status wrong,order:~w",[QueryString])
			end;
		_ ->
			db_func:log_pay_info(2, 0, 0, 0, erlang:localtime(), 0, ?srcType, QueryString),
			Reply = "FAILED",
			Req:ok({"text/html; charset=utf-8", Reply}),
			?ERR("yyh check_order failed. reason:sign wrong,order:~w",[QueryString])
	end.
