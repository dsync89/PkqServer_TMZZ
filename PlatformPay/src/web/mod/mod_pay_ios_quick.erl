...
-module(mod_pay_ios_quick).
-export([handle/1]).
-include("common.hrl").
-include("record.hrl").

%%md5key
-define(APPKEY, "1u9qkoxtyoayhqfgdgkeg2f4kj3ndplf").
...
handle(Req) ->
    ...
    case check_sign(Req) of
        {true,ServerID,RoleID,Amount,QueryList,_} ->
            ...
            URL = "http://"++Server#server.serverIP++":"++HttpPort++"/pay_quickios?"++SendQS,
			?ERR("pay quick ios, game server, URL = ~p~n", [URL]),
...
check_sign(Req) ->
    ...
    ?DEBUG("debug quick ios pay, QueryList = ~p~n", [QueryList]),
    ...
    case (not lists:member(?undefined, [Nt_data,Md5Sign,Sign])) of
        false ->
            ?ERR("quick ios pay, query list error, QueryList = ~p~n", [QueryList]),
            Reply = "FAILED",
        ...
        true ->
            case sign(Nt_data,Sign) =:= Md5Sign of
                false ->
                    ...
                    ?ERR("quick ios pay, check sign error, QueryList = ~p~n", [QueryList]),
                    Reply = "FAILED",
                    Req:ok({"text/html; charset=utf-8",Reply}), false;
                true ->
					?DEBUG("debug quick ios pay, singn ====true "),
					%%金额
					...
					Amount = Amount2*800,
					?DEBUG("debug quick ios pay, servier = ~p~n", [ServerID]),
					?DEBUG("debug quick ios pay, RoleID = ~p~n", [RoleID]),
					?DEBUG("debug quick ios pay, Amount2 = ~p~n", [Amount]),
...
data_decode(NData) ->
	...
	Key = "47118261714193875250097082694339",
...
%%	NL = lists:flatten(lists:duplicate(3,NL0)),
	...
	?DEBUG("debug quick ios pay, OutString = ~p~n", [OutString]),
	OutString.
