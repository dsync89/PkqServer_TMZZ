...
enter_file(NewName, Inc, From, St) ->
    case file_op(path_open,[St#epp.path, NewName, [read]]) of
	{ok,NewF,Pname} ->
...
scan_toks(From, St) ->
    case file_op(io, scan_erl_form, [St#epp.file, '', St#epp.location]) of
	{ok,Toks,Cl} ->
	...
	{error,E} ->
		io:format("io:scan_erl_form/3 error=~10000p, args=~w, info=~w\n",[E, 
																		  [St#epp.file, '', St#epp.location], get_process_info(St#epp.file)]),
