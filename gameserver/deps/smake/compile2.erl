...
format_error({module_name,Mod,Filename}) ->
    io_lib:format("Module name '~s' does not match file name '~s'",
		  [Mod,Filename]);
format_error(Error) ->
	io_lib:format("compile2 error :~10000p",[Error]).
...
remove_file(St) ->
    file_op(delete, [St#compile.ofile]),
    {ok,St}.
...
beam_consult_asm(St) ->
    case file_op(consult, [St#compile.ifile]) of
	{ok, Forms0} ->
...
read_beam_file(St) ->
    case file_op(read_file, [St#compile.ifile]) of
	{ok,Beam} ->
...
parse_module(St) ->
    ...
    R =  epp2:parse_file(St#compile.ifile, IncludePath, pre_defs(Opts)),
    case R of
	{ok,Forms} ->
...
makedep_output(#compile{code=Code,options=Opts,ofile=Ofile}=St) ->
    ...
    Ret = case Output0 of
		  ...
		  case file_op(delete, [Output0]) of
		      Ret2 when Ret2 =:= ok; Ret2 =:= {error,enoent} ->
			  case file_op(open,[Output0, [write]]) of
			      {ok,IODev} ->
				  {ok,IODev,true};
			      {error,Reason2} ->
				  {error,open,Reason2}
	...
	{ok,Output1,CloseOutput} ->
		...
		if
		    CloseOutput -> file_op(close,[Output1]);
		    true -> ok
		end,
		{ok,St}
	    catch
...
save_binary_1(St) ->
	...
	ok ->
	    case file_op(rename,[Tfile, Ofile]) of
		ok ->
		    {ok,St};
		{error,RenameError} ->
		    Es0 = [{Ofile,[{?MODULE,{rename,Tfile,Ofile,
					     RenameError}}]}],
		    Es = case file_op(delete,[Tfile]) of
			     ok -> Es0;
			     {error,DeleteError} ->
				 Es0 ++
				     [{Ofile,
...
write_binary(Name, Bin, St) ->
    ...
    case file_op(write_file,[Name, Bin, Opts]) of
	ok -> ok;
	{error,_}=Error -> Error
    end.
...
listing(LFun, Ext, St) ->
    ...
    case file_op(open,[Lfile, [write,delayed_write]]) of
	{ok,Lf} ->
...
restore_expanded_types("P", Fs) ->
    epp2:restore_typed_record_fields(Fs);
restore_expanded_types("E", {M,I,Fs0}) ->
    Fs1 = restore_expand_module(Fs0),
    Fs = epp2:restore_typed_record_fields(Fs1),
    {M,I,Fs};
...
file_op(FunName, Args) ->
	file_op(file2, FunName, Args).
file_op(Mod, FunName, Args) ->
	erlang:apply(Mod, FunName, Args).

%% 	Node = node(erlang:group_leader()),
%% 	case rpc:call(Node, Mod, FunName, Args) of
%% 		{'EXIT',_} = Reason ->
%% 			{error, Reason};
%% 		Result ->
%% 			io:format("\nfun=~w, node=~w, args=~w, result=~w\n",[FunName, Node, Args, Result]),
%% 			Result
%% 	end.