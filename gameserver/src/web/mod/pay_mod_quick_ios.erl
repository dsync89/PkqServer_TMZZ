...
check_sign(Req) ->
    ...
    case (not lists:member(?undefined, [Nt_data,Md5Sign,Sign])) of
        ...
        true ->
			?ERR("debug quick ios pay, singn ====true "),
			%%金额
			...
			Amount = Amount2*800,
			?ERR("debug quick ios pay, servier = ~p~n", [ServerID]),
			?ERR("debug quick  iospay, RoleID = ~p~n", [RoleID]),
			?ERR("debug quick ios pay, Amount2 = ~p~n", [Amount2]),
