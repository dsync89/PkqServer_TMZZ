...
check_is_mul({StartOpenDayNum, {_, _, _}=StartTime}, {EndOpenDayNum, {_, _, _}=EndTime}) ->
    {OpenDate, _} = data_setting:get(serverOpenTime),
...
role_id_to_server_id(RoleID) ->
    (RoleID div 1000000) - 1.

replace_nth(Nth, List, Elem) ->
    case erlang:length(List) >= Nth of
        true ->
            replace_nth(Nth, List, Elem, 1, []);
        false ->
            List
    end.

replace_nth(Nth, [_|List], Elem, Nth, HeadList) ->
    lists:reverse(HeadList) ++ [Elem|List];
replace_nth(Nth, [H|List], Elem, Count, HeadList) ->
    replace_nth(Nth, List, Elem, Count + 1, [H|HeadList]).

list_key_replace(Key1, Key2, N, List) ->
    case lists:keytake(Key1, N, List) of
        false ->
            case lists:keytake(Key2, N, List) of
                false ->
                    List;
                {value, Tuple2, ListLeft} ->
                    [erlang:setelement(N, Tuple2, Key1)|ListLeft]
            end;
        {value, Tuple1, ListLeft} ->
            case lists:keytake(Key2, N, ListLeft) of
                false ->
                    [erlang:setelement(N, Tuple1, Key2)|ListLeft];
                {value, Tuple2, ListLeft2} ->
                    [erlang:setelement(N, Tuple1, Key2), erlang:setelement(N, Tuple2, Key1)|ListLeft2]
            end
    end.
