%% @author admin
%% @doc @todo Add description to mod_login_zr.
...
handle(Req) ->
	...
	if SessionID =:= ?undefined orelse DevID =:= ?undefined ->
	   ...
	   true ->
		   case db_func:get_zr_accid(SessionID,DevID,util:get_ip_from_req(Req)) of
			   [] ->
				   Reply = ejson:encode({[{<<"result">>,1}]}),
				   platform_tool:return(Req, Reply);
			   [Accid] ->
                   Version = proplists:get_value("version", QueryString),
                   {DelServerIDList, ServerList} = tk_config:get_server_list(?ACCOUNT_TYPE_ZR, Version),
                   ...
                   pm_platform_server:add_loglogin(Accid, ?ACCOUNT_TYPE_ZR),
