...
handle(Req) ->
    ...
    if SessionID =:= ?undefined orelse DevID =:= ?undefined ->
       ...
       true ->
           case db_func:get_txhd_ard_ext_accid(SessionID,DevID,util:get_ip_from_req(Req)) of
               [] ->
                   Reply = ejson:encode({[{<<"result">>,1}]}),
                   platform_tool:return(Req, Reply);
               [Accid] ->
                   Version = proplists:get_value("version", QueryString),
                   {DelServerIDList, ServerList} = tk_config:get_server_list(?ACCOUNT_TYPE_TXHD_ARD_EXT, Version),
                   ...
                   pm_platform_server:add_loglogin(Accid, ?ACCOUNT_TYPE_TXHD_ARD_EXT),
