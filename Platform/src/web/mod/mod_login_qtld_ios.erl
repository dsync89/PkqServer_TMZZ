...
handle(Req) ->
    case check_auth(Req) of
        ...
        {true,Uid,DevID,Version} ->
            case db_func:get_qtld_ios_accid(Uid, DevID,util:get_ip_from_req(Req)) of
                [] ->
                    Reply = ejson:encode({[{<<"result">>,1}]}),
                    Req:ok({"text/html; charset=utf-8",Reply});
                [Accid] ->
                    {DelServerIDList,ServerList} = tk_config:get_server_list(?ACCOUNT_TYPE_QTLD_IOS, Version),
                    ...
                    pm_platform_server:add_loglogin(Accid, ?ACCOUNT_TYPE_QTLD_IOS),
                    Reply = ejson:encode({[
                                            {<<"result">>,0},
                                            {<<"accid">>,Accid},
...
check_auth(Req) ->
    ...
    ?ERR("debug login qtld_ios, QueryList = ~p~n", [QueryList]),
%%     Pid = proplists:get_value("pid", QueryList),
    ...
    case (not lists:member(?undefined, [UserID,SessionID,DevID,Version])) of
        false ->
            ?ERR("qtld_ios login, query list error, QueryList = ~p~n", [QueryList]), false;
        true ->
            ...
            case httpc:request(post, {URL,[],CType,Body}, [], []) of
                {ok,{_,_,Ret}} ->
                    case Ret of
                        ...
                        _ ->
                            ?ERR("qtld_ios login, check ret error,Ret = ~p~n", [Ret]), false
                    end;
                _ ->
                    ?ERR("qtld_ios login, url request error, URL = ~p~n", [URL]), false
            end
    end.
