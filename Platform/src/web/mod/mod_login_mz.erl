    ...
    case lists:member(?undefined, [AppID, Token, DevID, Version, Uname]) of
        false ->
            case check_auth(AppID, Token) of
                {true, Uname} ->
                    case db_func:get_mz_accid(Uname, DevID,util:get_ip_from_req(Req)) of
                        [] ->
                            Reply = ejson:encode({[{<<"result">>, 1}]}),
                            Req:ok({"text/html; charset=utf-8", Reply});
                        [Accid] ->
                            {DelServerIDList, ServerList} = tk_config:get_server_list(?ACCOUNT_TYPE_MZ, Version),
                            ...
                            pm_platform_server:add_loglogin(Accid, ?ACCOUNT_TYPE_MZ),
                            Reply = ejson:encode({[
                                                    {<<"result">>, 0},
                                                    {<<"accid">>, Accid},
                ...
                {false, Reason} ->
                    ?ERR("mz login check failed, Reason:~w", [Reason]),
