    ...
    if UID =:= ?undefined orelse SessionID =:= ?undefined orelse DevID =:= ?undefined ->
       ...
       true ->
           case check_auth(SessionID) of
               true ->
                   case db_func:get_bd_accid(UID, DevID,util:get_ip_from_req(Req)) of
                       [] ->
                           Reply = ejson:encode({[{<<"result">>,1}]}),
                           platform_tool:return(Req, Reply);
                       [Accid] ->
                          Version = proplists:get_value("version", QueryString),
                          {DelServerIDList, ServerList} = tk_config:get_server_list(?ACCOUNT_TYPE_BAIDU, Version),
                          ValidTime = integer_to_list(util:now()+ ?KEY_VALID_INTERVAL),
                           ...
                           pm_platform_server:add_loglogin(Accid, ?ACCOUNT_TYPE_BAIDU),
...
check_auth(SessionID) ->
    AppIDStr = "5009295",
    AppKey = "ZuEK6k8H6fPnG3E6GO6rDa0wBXjVM2FI",
    Sign = sign([AppIDStr, SessionID, AppKey]),
    URL = "http://querysdkapi.91.com/LoginStateQuery.ashx",
    Body = "AppID="++AppIDStr++"&AccessToken="++SessionID++"&Sign="++Sign,
