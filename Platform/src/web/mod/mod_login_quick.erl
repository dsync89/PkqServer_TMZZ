...
handle(Req) ->
    case check_auth(Req) of
        ...
        {true,Uid,DevID,Version} ->
            case db_func:get_quick_accid(Uid, DevID,util:get_ip_from_req(Req)) of
                [] ->
...
check_auth(Req) ->
    ...
    ?DEBUG("debug login quick, QueryList = ~p~n", [QueryList]),
    ...
    case (not lists:member(?undefined, [App,Sdk,Uin,Sess,DevID,Version])) of
        false ->
            ?ERR("quick login, query list error, QueryList = ~p~n", [QueryList]), false;
        true ->
            URL = "http://checkuser.sdk.quicksdk.net/v2/checkUserInfo?"
                    ++mochiweb_util:urlencode(
						[{"token",Sess},{"uid",Uin},{"product_code","40601024767531290510654112404838"}]),
            ...
            case httpc:request(get, {URL, []}, [], []) of
                {ok,{_,_,Ret}} ->
                    case Ret of
                        ...
                        _ ->
                            ?ERR("quick login, check ret error,Ret = ~p~n", [Ret]), false
                    end;
                _ ->
                    ?ERR("quick login, url request error, URL = ~p~n", [URL]), false
            end
    end.
